...
def save_clean_mask(sample, output_path, input_image=None, overlay_path=None, threshold=0.5, min_area=100):
    ...
    if input_image and overlay_path:
        ...
        overlay_mask = np.array(Image.fromarray(cleaned_mask).resize(overlay.shape[:2][::-1], Image.NEAREST))
...
def main():
    ...
    for subdir in sorted(os.listdir(args.data_dir)):
        sub_path = os.path.join(args.data_dir, subdir)
        ...
        dataset = ImageFolderDataset(sub_path, transform=transform)
        if len(dataset) == 0:
            print(f"⚠️ No images in {sub_path}")
            continue

        sampler = th.utils.data.distributed.DistributedSampler(dataset, shuffle=False)
        dataloader = th.utils.data.DataLoader(dataset, batch_size=args.batch_size,
        ...
        for b, paths in dataloader:
            ...
            for i in range(args.num_ensemble):
                ...
                for j in range(b.shape[0]):
                    ...
                    imageio.imwrite(os.path.join(out_subdir, f"{slice_ID}_sample{i}_crack_prob.png"), crack_prob)
                    argmax_mask = (softmax_probs[j].argmax(0).cpu().numpy() * 255).astype(np.uint8)
                    imageio.imwrite(os.path.join(out_subdir, f"{slice_ID}_sample{i}_argmax_mask.png"), argmax_mask)
                    save_clean_mask(
                        sample=softmax_probs[j:j+1],
                        output_path=os.path.join(out_subdir, f"{slice_ID}_sample{i}_binary_mask.png"),
                        input_image=paths[j],
                        overlay_path=os.path.join(out_subdir, f"{slice_ID}_sample{i}_overlay.png"),
                        threshold=0.5, min_area=80
                    )
