...
class DPM_Solver:
    ...
    def dynamic_thresholding_fn(self, x0, t):
        """
        The dynamic thresholding method (batch-wise).
        """
        B = x0.shape[0]
        p = self.dynamic_thresholding_ratio

        # 逐图像计算阈值 s_i
        s = torch.quantile(torch.abs(x0).reshape(B, -1), p, dim=1, keepdim=True)
        s = torch.maximum(s, self.thresholding_max_val * torch.ones_like(s).to(s.device))  # shape [B, 1]

        # 扩展维度为 [B, 1, 1, 1] 以进行广播裁剪
        while s.dim() < x0.dim():
            s = s.unsqueeze(-1)
