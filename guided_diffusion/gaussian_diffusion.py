...
def get_named_beta_schedule(schedule_name, num_diffusion_timesteps): #加入噪声计划
    #linear 线性等差序列
    #cosine 前期慢慢加、后期加多点 在训练中表现通常更好、更平滑（更慢的噪声退化，保留更多细节）
    """
...
class ModelVarType(enum.Enum): #控制“方差”的预测方式（variance schedule）
    """
    ...
    LEARNED = enum.auto()  #模型直接输出 σ²（或者 log σ²）	更灵活，训练目标更多样，但难度更大
    FIXED_SMALL = enum.auto()  #使用训练前预设的一组较小的方差值	默认选项，生成图更清晰、收敛更快
    FIXED_LARGE = enum.auto()  #使用较大的固定方差值（posterior 真实值）	训练更稳定，但采样易糊
    LEARNED_RANGE = enum.auto() #模型输出 [-1, 1] 的系数，插值在 fixed small 和 fixed large 之间 稳定性更好，是 LEARNED 的改良版

    # EPSILON + LEARNED_RANGE  for segmentation


class LossType(enum.Enum): #控制模型的训练目标函数
    MSE = enum.auto()  # use raw MSE loss (and KL when learning variances) 标准的均方误差 loss（预测 ε vs GT ε）	图像生成 / mask预测 
    RESCALED_MSE = (
        enum.auto()
    )  # use raw MSE loss (with RESCALED_KL when learning variances) MSE + VLB 中的 KL 项（但 KL 比例调小）	高质量生成时稳定训练
    KL = enum.auto()  # use the variational lower-bound 使用变分下界 loss（VLB），完全基于 KL	VAE-like diffusion、需要 log likelihood 的任务
    RESCALED_KL = enum.auto()  # like KL, but rescale to estimate the full VLB与 KL 类似，但做了缩放估计真实 NLL	主要用于评估模型 bits/dim 性能
    BCE_DICE = enum.auto() #特别为 segmentation 设计的 loss：BCE + Dice	医学分割 / mask prediction
...
class GaussianDiffusion:
    ...
    def q_mean_variance(self, x_start, t): 
        #模拟前向扩散过程（加噪）
        #输入 原始图像 x_start，第 t 步
        #输出 mean 加噪后的方差 variance 方差 log_variance  方便后续计算 KL 或 likelihood
        """
    ...
    def q_sample(self, x_start, t, noise=None):
        # 从原始图像 x_0 加噪，生成 x_t（用于训练中的输入），把干净图像打上马赛克（加噪声）
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t): 
        #在已知 x_t 和原图 x_0 的情况下，理论上我们知道 x_{t-1} 会是什么分布
        """
    ...
    def p_mean_variance(
        ...
        #我现在在第 t 步拿到一个 noisy 图 x_t，我用模型预测 ε 或 x₀，再反推出第 t-1 步的图像分布和原图预测 推理阶段
        """
    ...
    def _predict_eps_from_xstart(self, x_t, t, pred_xstart):
        #已知当前的 noisy 图像 $x_t$ 和预测的干净图像 $x_0$，反推出模型本应该预测的 $\epsilon$
        return (
            _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.shape) * x_t
            - pred_xstart
...
#-------------------------不需要condition_mean and condition_score---------------------
...
#-------------------------不需要condition_mean and condition_score---------------------
    ...
    #这是调用 diffusion 采样流程的入口函数，用于从已有图像 img 出发，通过条件扩散生成输出（如 mask

    def p_sample(
            #给定当前 noisy 图像 x_t 和当前时间步 t，用模型预测出 $x_0$，然后构造高斯分布 $p(x_{t-1} \mid x_t)$，并从中采样出 $x_{t-1}$。
        self,
        model,
        x,
        t,
...
#--------------------采样主流程----------------------------
#从高斯噪声开始，逐步调用 p_sample() 采样，从 $x_T$ 一步步还原出 $x_0$，最终生成图像（如 mask）
    ...
    #给定一张原始图像 img，通过 diffusion 模型生成预测的 segmentation mask 
    # 并且自动处理条件拼接、噪声注入、DPM-Solver 或 DDPM 采样器、后处理融合结果
    # 输入 原始图像  输出 预测 mask、校准 mask、融合 mask
    #预测时调用的函数
    #我给你一张图，你用 diffusion 逐步采样，生成一个结构清晰的掩膜图，并结合校准 mask 得到最终结果
        self,
        model,
        shape,
        img,
    ...
    def training_losses_segmentation(self, model, classifier, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.BCE_DICE or self.loss_type == LossType.RESCALED_MSE:
            ...
            terms["loss_cal"] = F.binary_cross_entropy_with_logits(cal, res.float())
