...
class UNetModel_newpreview(nn.Module):
    ...
    def __init__(
        ...
        #----------------middle_block 介于encoder and decoder, bottleneck ----------------------
        self.middle_block = TimestepEmbedSequential(
            ResBlock(
                ch,
                time_embed_dim,
    ...
    def forward(self, x, timesteps, y=None):
        ...
        h = x.type(self.dtype)  #[1,4,256,256]
        c = h[:,:-1,...]  # [N,3,H,W] RGB [1,3,256,256]

        anch, cal = self.highway_forward(c) 
        #对RGB做特征提取 结构引导 anchor_out [1, 64, 256, 256] 	用于拼到 UNet 
        #seg_output [1, 1, 256, 256] cal 结构图
...
class Generic_UNet(SegmentationNetwork):
    ...
    def forward(self, x, hs = None):
        ...
        for d in range(len(self.conv_blocks_context) - 1):
            x = self.conv_blocks_context[d](x)  #堆叠了一系列层 [1,32,256,256]
            skips.append(x) # [1,32,256,256]
            if not self.convolutional_pooling:
                x = self.td[d](x)  # 下采样 [1,64,128,128]
            if hs:
                h = hs.pop(0)
                ddims = h.size(1)
        ...
        x = self.conv_blocks_context[-1](x)  #Bottleneck  [1,64,128,128]
        emb = conv_nd(2, x.size(1), 512, 1).to(device = x.device)(x) # [1,512,128,128]

        for u in range(len(self.tu)):
            x = self.tu[u](x) #上采样 [1,64,256,256]
            x = th.cat((x, skips[-(u + 1)]), dim=1) # 拼接 skip [1, 96, 256, 256]
            x = self.conv_blocks_localization[u](x) # [1, 64, 256, 256]
            if self._deep_supervision:
                seg_outputs.append(self.final_nonlin(self.seg_outputs[u](x))) #[1, 1, 256, 256]  ← 结构感知图（cal）
            if self.anchor_out and (not self._deep_supervision):
                anch_outputs.append(x) # [1, 64, 256, 256] ← anchor 特征
        if not seg_outputs:
